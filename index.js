...
module.exports = (job, settings, { provider, params, ...options }, type) => {
    ...
    let input = `aerender-${job.uid}.log`
    ...
    if (!path.isAbsolute(input)) input = path.join(settings.workpath, input);
    ...
    settings.logger.log(`[${job.uid}] starting action: upload log file`)
